...
(fn-spec location-accessible-to? [::dmap ::location ::unit-type] boolean?)
...
(fn-spec locations-colocated? [::dmap ::location ::location] boolean?)
...
(fn-spec colocation-set-for-location [::dmap ::location] (s/coll-of ::location))
...
(fn-spec supply-center? [::dmap ::location] boolean?)
(defn supply-center?
  [diplomacy-map location]
  (contains? (:supply-centers diplomacy-map) location))

(fn-spec home-supply-centers [::dmap ::country] boolean?)
...
(fn-spec create-order ::create-order-args ::order)
(defn create-order
  ...
  (let [basic-order {:country  country  :unit-type  unit-type
        ...
        ;; match/`match` won't let multiple patterns map to the same expression,
        ;; so we put the expression in a thunk to avoid duplication.
        make-assisting-order
        (fn [] (assoc basic-order
                      :assisted-order (apply create-order rest)))]
    (match/match [unit-type order-type rest]
...
(fn-spec army?    [::order] boolean?)
(fn-spec fleet?   [::order] boolean?)
(fn-spec hold?    [::order] boolean?)
(fn-spec attack?  [::order] boolean?)
(fn-spec support? [::order] boolean?)
(fn-spec convoy?  [::order] boolean?)
...
(fn-spec next-intended-location [::order] ::location)
...
(fn-spec locations-used-by-order [::order] (s/coll-of ::location))
